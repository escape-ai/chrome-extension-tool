...
function getArticleInfo(tab) {
    let articleInfo;
    let url = getUrl(tab);
    let hashedUrl = md5(url);
    console.log(`Hashed URL = ${hashedUrl}`);
    let articleByUrlEndpoint = `${API_ROOT}articles/articlebyurl/${hashedUrl}/`;
    let xmlhttp = new XMLHttpRequest();
    xmlhttp.open("GET", articleByUrlEndpoint, true);
    xmlhttp.onreadystatechange = function () {
        if (this.readyState === 4) {
            if (this.status === 200) {
